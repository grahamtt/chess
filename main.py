...
def main(page: ft.Page):
    ...
    def execute_human_move(from_row, from_col, to_row, to_col, animate=True):
        ...
        if is_promo and game_mode == "antichess" and isinstance(game, AntiChessGame):
            _show_antichess_promotion_dialog(from_row, from_col, to_row, to_col)
            return True
        ...
        _complete_move(from_row, from_col, to_row, to_col, animate=animate)
        return True

    def _complete_move(
        from_row, from_col, to_row, to_col, promotion=None, animate=True
    ):
            ...
            buttons.append(
                ft.TextButton(
                    content=ft.Row(
                        [
                            ft.Image(
                                src=svg, width=32, height=32, fit=ft.BoxFit.CONTAIN
                            ),
            ...
            if game.is_antichess_game_over():
                ...
                if result == "1-0":
                    message.current.value = (
                        "White wins! (lost all pieces or stalemated)"
                    )
                    message.current.color = ft.Colors.BLUE
                    game_over = True
                    ...
                    message.current.value = (
                        "Black wins! (lost all pieces or stalemated)"
                    )
                    message.current.color = ft.Colors.BLUE
                    game_over = True
